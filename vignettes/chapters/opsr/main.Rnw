...
The model \code{fit\_intercept} was excluded from the \fct{texreg::screenreg} call on Page~\pageref{p:omitted-fit-intercept}.
}%
...
%
\begin{multline} \label{eq:log-cond-exp}
\E[Y_j \mid Z = j] =
...
%
\begin{multline} \label{eq:log-counterfact-exp}
\E[Y_{j'} \mid Z = j] =
