...
\begin{table}[htbp]
\centering
<<tableone, echo=FALSE, results=tex>>=
tab1 <- TWTE::tableone
...
\centering
\resizebox{\textwidth}{!}{%
<<table-wfh, echo=FALSE, results=tex>>=
tab <- TWTE::table_wfh
...
\begin{figure}[htbp]
\centering
<<tw-comparison, echo=FALSE, fig=TRUE, height=5.3, width=5.3>>=
wfh_survey <-
  TUplus::participants %>%
...
\begin{table}[htbp]
\centering
<<tabletwo, echo=FALSE, results=tex>>=
tab2 <- TWTE::tabletwo
...
In the following, we outline the ordered probit switching regression model as well as list all the key formulas enabling the computation of treatment effects. Section~\ref{sec:the-model} and Section~\ref{sec:the-conditional-expectation} are (slightly) modified versions of \citet{Heimgartner+Wang:2025}.

\subsubsection{The model} \label{sec:the-model}
...
\subsubsection{The conditional expectation} \label{sec:the-conditional-expectation}
...
The OPSR methodology is implemented in the \pkg{OPSR} \proglang{R}-package \citep{Heimgartner+Wang:2025} and is available from the Comprehensive \proglang{R} Archive Network (CRAN) at \url{https://CRAN.R-project.org/package=OPSR}.
...
<<hidden-weekday-shares, echo=FALSE, results=hide>>=
dat <-
  TWTE::work %>%
...
%
<<hidden-activity-sequences, echo=FALSE, results=hide>>=
as <-
...
plot.it <- function() {
  ...
  png(file, height = 11, width = 11, units = "in", res = 2 * 144)
...
\begin{figure}[htbp]
\centering
<<entropies-and-shares, echo=FALSE, fig=TRUE, height=7.5, width=7.9>>=
...
\begin{figure}[htbp]
\centering
<<disaggregate-time-use, echo=FALSE, fig=TRUE, height=9, width=6.5>>=
plot.it <- function() {
  op <- par(no.readonly = TRUE)
  on.exit(par(op))
  par(mfrow = c(3, 2))
...
\begin{figure}[htbp]
\centering
<<disaggregate-mobility-indicators, echo=FALSE, fig=TRUE, height=9.5, width=6.5>>=
plot.it <- function() {
  op <- par(no.readonly = TRUE)
  on.exit(par(op))
  par(mfrow = c(3, 2))
...
\begin{figure}[htbp]
\centering
<<aggregate-time-use, echo=FALSE, fig=TRUE, height=9, width=6.5>>=
plot.it <- function() {
  op <- par(no.readonly = TRUE)
  on.exit(par(op))
  par(mfrow = c(3, 2))
...
\begin{figure}[htbp]
\centering
<<aggregate-mobility-indicators, echo=FALSE, fig=TRUE, height=9.5, width=6.5>>=
plot.it <- function() {
  op <- par(no.readonly = TRUE)
  on.exit(par(op))
  par(mfrow = c(3, 2))
...
<<hidden-r2, echo=FALSE, results=hide>>=
dat <- TWTE::data_model
weights <- dat$weight
...
\midrule
<<model-km-fit, echo=FALSE, results=tex>>=
...
mat_tex[c(42, 51)] <- paste(mat_tex[c(42, 51)], "\\midrule")
writeLines(mat_tex)
@
\bottomrule
\end{tabular}
...
\begin{figure}[htbp]
\centering
<<model-km-pairs, echo=FALSE, fig=TRUE, height=7.5, width=7.5>>=
...
<<hidden-ce, echo=FALSE, results=hide>>=
...
\begin{table}[htbp]
\centering
<<model-km-ate, echo=FALSE, results=tex>>=
...
\begin{table}[htbp]
\centering
<<model-km-ute, echo=FALSE, results=tex>>=
...
\midrule
<<tab-no-cor, echo=FALSE, results=tex>>=
dat <- TWTE::data_model
weights <- dat$weight
...
\proglang{R}~\Sexpr{paste(R.Version()[6:7], collapse = ".")} \citep{R} with the packages \pkg{OPSR}~\Sexpr{packageVersion("OPSR")} \citep{Heimgartner+Wang:2025} and \pkg{TraMineR}~\Sexpr{packageVersion("TraMineR")} \citep{Gabadinho+Etal:2011}. \pkg{tableone}~\Sexpr{packageVersion("tableone")} \citep{Yoshida+Bartel:2022}, \pkg{knitr}~\Sexpr{packageVersion("knitr")} \citep{Yihui:2024} and \pkg{kableExtra}~\Sexpr{packageVersion("kableExtra")} \citep{Zhu:2024} were helpful to generate and format some of the tables. \proglang{R} itself
and all packages used are available from the Comprehensive
\proglang{R} Archive Network (CRAN) at
\url{https://CRAN.R-project.org/}.

Some post-estimation routines developed as part of this work will become available in the \pkg{OPSRtools} package in the near future.

The authors reorganized, cleaned and collated the TimeUse+ data in the \pkg{TUplus}~\Sexpr{packageVersion("TUplus")} \proglang{R}-package. Both the original data and the package can be shared for research purposes. Please contact the corresponing author.


\section*{Acknowledgments}

The TimeUse+ study was funded by the Swiss National Science Foundation (SNSF) (Grant Number 189001).
...
\renewcommand{\arraystretch}{1}  % default row height
\begin{center}
\begin{ThreePartTable}
\begin{TableNotes}[flushleft]
\item Signif. codes: 0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1;
...
\endlastfoot
<<model-km-comparison, echo=FALSE, results=tex>>=
