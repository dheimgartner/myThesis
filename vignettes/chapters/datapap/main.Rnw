...
\todo{%
\begin{itemize}
\item Maybe remove response rate analysis section as it is quite redundant
\item Don't mention bootstrap stuff
...
Before the pandemic, working from home was rare and often stigmatized, seen as detrimental to career advancement, which discouraged workers from expressing their desire to telework \citep{brewer2000distributed}. Indeed, \citet{mokhtarian1996two} found that telework was a \textit{preferred impossible} option for many workers. The pandemic, however, helped break this stigma and relaxed some constraints, making telework a \textit{preferred possible} scenario for many.
...
Attributes and their levels are presented in \cref{tab:wfh-attributes-levels}. Notably, three attributes (\textit{hardware budget, additional cost}, and \textit{salary adjustments}) imply a cost component and a marginal utility of one Swiss franc. It is to be tested whether the monetary utility equivalent remains constant across these three attributes. The \textit{coordination} attribute could have both positive and negative utility implications: While it reduces personal flexibility, it also coordinates office attendance, enhancing collaboration and collegiality. The \textit{desk sharing} attribute captures the concept of flexible office space utilization and expands on \citet{appel2022attract} where work location attributes such as \textit{noise, openness} and \textit{crowdedness} were tested and found to be important. The \textit{help and training} attribute implies support for technical difficulties and training, promoting effective digital collaboration and a successful home office culture. The full factorial design was reduced according to D-efficiency principles. Each participant completed four choice tasks.

\input{tables/wfh_attributes_levels}
...
\begin{figure}
    \centering
    \fbox{\includegraphics[width=3.5in]{figures/wfh_choice_task.png}}
    \caption{Example of a choice task for the work from home stated preference experiment.}
    \label{fig:wfh-choice-task}
\end{figure}
...
\begin{figure}
    \centering
    \fbox{\includegraphics[width=3.5in]{figures/mto_choice_task.png}}
    \caption{Example of an individualized choice task for the mobility tool ownership stated preference experiment.}
    \label{fig:mto-choice-task}
\end{figure}
...
This section examines the demographic composition of survey participants, discusses the relationship between response burden \citep{axhausen2015predicting} and response rates, and explores the variability in choices observed in the two SP experiments.

As previously mentioned, the survey instrument in \textit{Stage I} targeted the entire working population, while SP experiments focused exclusively on those eligible for telework. The distribution across cantons, completion rates, and response rates for \textit{Stage I} are depicted in \cref{fig:cantonal-distribution}. To emphasize variability among cantons, Zurich, which was oversampled, is excluded from the linear color scale of the first two panels. The remaining German-speaking cantons are included, with invited participants proportionally reflecting their respective populations. Response rates vary across cantons, ranging from 14.29\% (Glarus) to 34.10\% (Lucerne).

\begin{figure}
<<fig.height=7>>=
cantonal_distribution$plot
@
\caption{Distribution of invited participants, survey completion, and response rates across cantons. Zurich is excluded from the color scale in the first two panels to highlight variation among other cantons.}
\label{fig:cantonal-distribution}
\end{figure}

\input{tables/mzmv_marginals}

\Cref{tab:mzmv-marginals} presents marginal distributions of shared variables for the MTMC21 sample, the pre-study, and the main study populations. The main study sample closely mirrors the MTMC21 sample with some exceptions, likely attributable to Zurich's oversampling. Overrepresentation is noted among high-income households and PT season-ticket holders. Regarding ownership of mobility tools, comparability of marginals is limited due to differing survey questions: MTMC21 assessed access to tools while ours inquired about ownership and regular usage. Marginal distributions across reported NOGA sectors diverge notably, reflecting Zurich's status as a financial and ICT hub. Disparities in \textit{Telework} distributions are expected, influenced by the ongoing COVID-19 pandemic during the MTMC21 and pre-study survey periods, detailed further in \cref{sec:descriptive-findings}.

Despite successful sampling, a re-weighting scheme was implemented to enhance sample representativeness. Using iterative proportional fitting, weights were adjusted based on specified variables, excluding \textit{Telework} and \textit{Mobility Tools} due to aforementioned limited comparability. These weights were applied to aggregate statistics whenever findings were generalized to the entire population, employing the \pkg{anesrake} package \citep{anesrake} for weight determination.

Building on \citet{axhausen2019predicting}, we estimated a linear regression model relating response burden scores to log-transformed response rates:
...
where $y_i$ denotes the response rate (in percent), $x_i$ represents the ex-ante response burden score, and $\varepsilon_i$ is a normally distributed clustered error term. Observations were weighted by the square root of the sample size. The model was estimated for the entire sample and separately for recruitment by incentive status (\textit{Pre-study:} No recruitment, no incentive; \textit{Stage I (all):} No recruitment, no incentive; \textit{Stage II and III:} Recruitment and incentive).

<<results='asis'>>=
cat(response_rate_analysis$table_both)
@

In comparison to the previous publication \citep{schmid2019predicting}, 14 additional surveys contributed by members of the \textit{Institute of Transport Planning and Systems} (IVT) at ETH Zurich were included. Updated model estimates with clustered standard errors beside the estimates based on the last publication sample are reported in \cref{tab:response-rate-analysis-both}. A new group (no recruitment but with incentive payments) is now part of the sample, however the effect of the response burden is not significant because of limited sample size. Due to the logistic transformation, parameters reflect changes in log-odds with marginal increases in response burden scores. The exponential $\exp(\beta_1)$ represents a marginal change in odds ratio (i.e., participation vs. non-participation). The back-transformed relationship between response burden and response rates is visualized in \cref{fig:response-rate-analysis}, alongside 95\% confidence intervals. Commenting on the estimates, the effect of response burden is generally more negative than previously expected. The strongest effect can be found for the recruited subsample without incentive payment (\textit{Yes, no}), where the odds of participating decrease by $(\exp(\beta_1/1000)-1)*100 = -0.298$, if the response burden score increases by 100 points (i.e., roughly 1/3). Whereas previously, the pooled effect of an increase in the response burden of 100 points was reported to be 6\% it now is 10\% decrease in the odds.

\begin{figure}
<<out.width="4in", fig.width=5.5, fig.height=5.25>>=
response_rate_analysis$p
@
\caption{Updated relationship between response burden score and response rates.}
\label{fig:response-rate-analysis}
\end{figure}

Response rates closely approximate ex-ante predictions, with the exception of the \textit{pre-study}, falling below the 95\% confidence bound. A potential explanation could be fatigue among respondents sourced via an address dealer, possibly due to frequent solicitations. Addresses for the main study were sourced from the Federal Statistical Office's official sampling frame, with careful measures to avoid repeated sampling. Additionally, initial participation was likely influenced by potential incentives offered upon completion of all three survey stages.

Recruitment shifts the curve, while incentives flatten it. Notably, the domain above a response burden score of 2`000 is sparsely populated, and the few observations strongly influence the curve's shape. However, surveys beyond 2`000 points appear overly burdensome for respondents, sustaining high response rates only through recruitment efforts combined with incentive payments, intensive care of the respondents and general interest in the topic of these intense studies.
...
\begin{figure}
<<fig.height=4.25, fig.width=8>>=
wfh_choice_variation$p
@
\caption{Variation in telework choices and deviation from the free-choice scenario.}
\label{fig:wfh-choice-variation}
\end{figure}

Given respondents first selected preferred telework arrangements before specifying intended frequencies, the extent to which attribute-level balance is sustained in the frequency choices is questioned. \Cref{fig:wfh-attribute-level-balance} features separate panels for each attribute level, detailing mean telework frequencies on the left y-axis and frequency of attribute levels retained on the right y-axis. Attributes display generally balanced distributions, even in secondary frequency choices. Initial work arrangement choices filtered out some attributes such as salary decreases and restricted telework locations (\textit{work from anywhere}). The direction of the effects (blue lines) align with intuition: Attributes favorable to telework (e.g., salary increases) correlate with increased mean telework frequencies. The reported maximum difference (\textit{Max diff}) in each panel serves as an indicator for effect strength, highlighting salary adjustments and flexible telework locations as influential levers in telework behavior.

\begin{figure}
<<fig.width=8>>=
wfh_attribute_level_balance$p
@
\caption{Relation between attribute levels and aggregate telework supply (blue lines, left y-axis) and attribute level balance (orange lines, right y-axis).}
\label{fig:wfh-attribute-level-balance}
\end{figure}
...
\begin{figure}
<<fig.width=7.4, fig.height=5.75>>=
wfh_weekday_distribution$p
@
\caption{Distribution of telework shares across weekdays by number of teleworking days, for teleworking population only. Differentiating between full-time and part-time employed.}
\label{fig:wfh-weekday-distribution}
\end{figure}
...
@
\caption{Evolution of telework frequency shares, before, during and after the pandemic (panel a), and potential leeway for teleworking shares on weekdays (panel b). Workforce population only.}
\label{fig:wfh-share-simulation-leeway}
\end{figure}
...
\begin{figure}
<<fig.height=5.75, fig.width=14>>=
mto_wfh_variation$p
@
\caption{Comparison of mobility tool ownership shares by telework status and frequency. Panel a and b reflect ownership as of the survey period (June-August 2023), panel c is based on stated preference choices.}
\label{fig:mto-wfh-variation}
\end{figure}
...
bootstrap$p
@
\caption{Bootstrapped ownership shares based on probit models linking telework frequency to ownership choices (\cref{tab:bootstrap}). $95\%$ confidence intervals reflect parameter estimation uncertainties. ST denotes season ticket.}
\label{fig:bootstrap}
\end{figure}
