...
The remainder of the text is structured as follows: \cref{sec:survey-methods} details the survey methods and SP experiments. \cref{sec:response-behavior} analyzes response behavior and relates the response rate to the response burden score as proposed by \citet{Axhausen+Etal:2015}. \cref{sec:descriptive-findings} presents a descriptive analysis addressing who can, may, and wants to work from home. \cref{sec:telework-treatment-effects} demonstrates the usefulness of the data by investigating telework treatment effects on mobility tool ownership. Finally, \cref{sec:conclusion} concludes the study.
...
A series of Likert-scale questions was included to assess an individual's theoretical telework feasibility (\emph{teleworkability}). These questions encompassed job characteristics, the residential environment, and personality traits. This distinction allows modelers to differentiate between possibility and preference, an important concept emphasized by \citet{Mokhtarian+Salomon:1994}. A factor analysis was conducted on these items, with results discussed more in-depth in \cref{app:teleworkability-factor-analysis}. Two factors emerged as important: One representing job-related dimensions (work context and work activities), and the other relating to personal characteristics and the home office environment. These latent factors could be incorporated into modeling approaches, either directly or indirectly, such as through a probabilistic choice set formation approach as in Manski's model \citep[e.g.,][]{Manski:1977, Bierlaire+Etal:2010}.
