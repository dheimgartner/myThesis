...
\item 95\% confidence bounds to Figure~\ref{fig:bootstrap}
\item There is sometimes weird formatting going on at the linebreak in Table~\ref{tab:mzmv-marginals}
...
The section elaborating on the response rate analysis was shortened, as it largely overlaps with Chapter~\ref{ch:responseRateAnalysis}; Figure~\ref{fig:bootstrap} was improved and now shows the individual bootstrap predictions; The wording of the SPs introductory text was incorporated in the chapter (instead the Appendix); The remainder of the Appendix was omitted.
}%
...
The remainder of the text is structured as follows: Section~\ref{sec:survey-methods} details the survey methods and SP experiments. Section~\ref{sec:response-behavior} analyzes response behavior and relates the response rate to the response burden score as proposed by \citet{Axhausen+Etal:2015}. Section~\ref{sec:descriptive-findings} presents a descriptive analysis addressing who can, may, and wants to work from home. Section~\ref{sec:telework-treatment-effects} demonstrates the usefulness of the data by investigating telework treatment effects on mobility tool ownership. Finally, Section~\ref{sec:conclusion} concludes the study.
...
\begin{center}
(Figure~\ref{fig:wfh-choice-task})
\end{center}
...
\item The above choice (see Figure~\ref{fig:wfh-choice-task}) would imply that you would prefer work arrangement A over B and would like to work from home on 3 days a week given the conditions in arrangement A.
\end{enumerate}
\end{leftbar}
...
\begin{center}
(Figure~\ref{fig:mto-choice-task})
\end{center}
...
\item The above choice (see Figure~\ref{fig:mto-choice-task}) would imply that given the home office situation presented, you would choose to own the PT subscription and the bicycle.
\end{enumerate}
\end{leftbar}
...
Given these pandemic-driven shifts and the remaining potential, reconsidering the effect on mobility tool ownership is pertinent. As discussed in Section~\ref{sec:introduction-and-motivation}, this higher-order effect has been overlooked in previous studies. While group differences in ownership (panel a and b, Figure~\ref{fig:mto-wfh-variation}) suggest teleworkers more commonly own a half-fare card and slightly more often bicycles or E-bikes, car ownership and PT subscriptions are marginally lower among teleworkers. Panel b highlights distinct mobility tool compositions among those teleworking five or more days weekly, notably lower car and PT subscription ownership, the predominant commuting modes in Switzerland. However, simple group comparison might not portray the full picture as teleworkers likely constitute a special group with different mobility tool preferences even in the absence of the telework treatment (selection bias).

Therefore, clearer trends emerge from stated preference data where telework treatment was exogenous (see discussion in Section~\ref{sec:stage-iii}). Fully remote employees exhibit unique characteristics, evidenced by trends in (E-)bike and car ownership reversals, alongside linear trends in half-fare card and bike ownership increases, and declines in car ownership and PT subscriptions, particularly evident beyond four telework days weekly.
...
plot.it()
@
\caption{\label{fig:bootstrap} Bootstrapped ownership shares based on probit models linking telework frequency to ownership choices (based on the models presented in Table~\ref{tab:bootstrap}). Each line corresponds to the predictions based on one bootstrap sample. ST denotes season ticket.}
\end{figure}
