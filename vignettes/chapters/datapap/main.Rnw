...
Heimgartner D. and K. W. Axhausen (2024) Multimodality in the Swiss New Normal: Data Collection Methods and Response Behavior in a Multi-Stage Survey with Linked Stated Preference Designs, \emph{Transportation}, accepted for publication.
}%
\ChapterInfoEntry{
...
\begin{center}
\includegraphics[width=0.6\textwidth]{chapters/datapap/figures/wfh_choice_task}
\end{center}
...
\item The above choice would imply that you would prefer work arrangement A over B and would like to work from home on 3 days a week given the conditions in arrangement A.
\end{enumerate}
\end{mybox}
...
\begin{center}
\includegraphics[width=0.6\textwidth]{chapters/datapap/figures/mto_choice_task}
\end{center}
...
\item The above choice would imply that given the home office situation presented, you would choose to own the PT subscription and the bicycle.
\end{enumerate}
\end{mybox}
...
\begin{figure}[htbp]
\centering
<<datapap-weekday-dist, echo=FALSE, fig=TRUE, width=7.8, height=5.75>>=
