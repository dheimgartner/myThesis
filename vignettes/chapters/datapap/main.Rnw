...
\item Revise tables
\item Maybe adjust colors (don't use colors if not necessary)
\item Consistent footnotes (size and sig legend)
...
\begin{figure}[htbp]
\centering
\fbox{\includegraphics[width=0.8\textwidth]{chapters/datapap/figures/wfh_choice_task}}
...
\begin{figure}[htbp]
\centering
\fbox{\includegraphics[width=0.8\textwidth]{chapters/datapap/figures/mto_choice_task}}
...
% \input{chapters/datapap/tables/mzmv_marginals}
...
@
\caption{\label{fig:response-rate-analysis} Updated relationship between response burden score and response rates.}
\end{figure}

Response rates closely approximate ex-ante predictions, with the exception of the \emph{pre-study}, falling below the 95\% confidence bound. A potential explanation could be fatigue among respondents sourced via an address dealer, possibly due to frequent solicitations. Addresses for the main study were sourced from the Federal Statistical Office's official sampling frame, with careful measures to avoid repeated sampling. Additionally, initial participation was likely influenced by potential incentives offered upon completion of all three survey stages.
...
% \input{chapters/datapap/wfh_table_weighted_combined}
...
\begin{figure}[htbp]
\centering
<<echo=FALSE, fig=TRUE, height=6, width=13>>=
bootstrap$p
@
\caption{\label{fig:bootstrap} Bootstrapped ownership shares based on probit models linking telework frequency to ownership choices (\cref{tab:bootstrap}). $95\%$ confidence intervals reflect parameter estimation uncertainties. ST denotes season ticket.}
\end{figure}
