\chapter{\pkg{OPSRtools}: Estimation and Post-Estimation Routines for the \pkg{OPSR} Package}
\label{ch:opsr-tools}
...
\section{Introduction} \label{sec:opsr-tools-introduction}
...
\begin{figure}[htbp]
\centering
<<opsr-tools-plot-kfplot, echo=FALSE, fig=TRUE, height=3.5, width=8>>=
<<opsr-tools-kfplot>>
@
