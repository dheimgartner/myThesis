...
tex <-
  ...
  mutate(Item = str_replace_all(Item, "    ", "\\\\hspace{2em}")) %>%
  kableExtra::kbl(format = "latex",
                  caption = "Response burden: Points by question type and action",
                  label = "response_burden_scheme",
                  booktabs = TRUE,
                  escape = FALSE) %>%
  kableExtra::footnote(general = "Based on \\\\emph{Gesellschaft für Sozialforschung (GfS)}, Zürich, 2006 (updated).",
                       escape = FALSE,
...
\begin{figure}[htbp]
\centering
<<rra-response-burden-score, echo=FALSE, fig=TRUE, fig.width=3.75, fig.height=3.9>>=
rr <-
  response_rates %>%
...
@
\caption{\label{fig:response_burden_scores} Distribution of the response burden scores for \emph{recruitment x incentive} categories. \emph{RxI} stands for recruited, with incentives, \emph{N} negates.}
\end{figure}
...
tex <- texreg::texreg(m_both,
                      ...
                      custom.note = "%stars. \\dag Based on the sample of the last publication. \\emph{RxI} = Recruited, with incentives, \\emph{N} negates.",
                      scalebox = 0.75,
                      float.pos = "h!")
@
...
For the newly added group (no recruitment but with incentive payments, \emph{NRxI}) the effect of the response burden is not significant, as expected because of limited sample size. Due to the logistic transformation, parameters reflect changes in log-odds when the RBS marginally increases. The effect of response burden is generally more negative than previously expected. The strongest effect can be found for the recruited subsample without incentive payment (\emph{RxNI}), where the odds of participating decrease by -0.297 (i.e., roughly 30\%) according to \cref{eq:log_odds_decrease} if the RBS increases by 100 points. The other comparisons are listed in \cref{tab:pc_odds}.
...
\begin{figure}[htbp]
\centering
<<rra-response-curve, echo=FALSE, fig=TRUE, message=FALSE, fig.width=8, fig.height=3.8>>=
...
We repeat the steps for the individual categories and synthesise the results in \cref{tab:time_trend}. In contrast to \citet{schmid2019predicting} we do not find a negative time-trend and therefore do not support the hypothesis of a general fatigue and less willingness to participate in our surveys.
...
tex <- texreg::texreg(m_both,
                      ...
                      custom.note = "%stars. \\emph{RxI} = Recruited, with incentives, \\emph{N} negates.",
                      scalebox = 0.72,
                      float.pos = "h!")
@
