...
Most of the original text became part of the paper under review, underlying Chapter~\ref{ch:datapap} and is not repeated here. The original analysis modeled the telework frequency and used the baseline preference as explanatory variable. Here, we model the deviation from the baseline directly. Further, the random error component was added. While the qualitative conclusions did not change, modeling results did, with elasticities being even smaller. The methods section and the discussion of the results was adjusted accordingly.
}%
